    ...
    *,
    ...
    raise ProviderRateLimitError(
        ...
        rate_limit={
            ...
            "raw_type": reason,
            "raw_log_path": event.get("raw_log_path") or data.get("raw_log_path"),
        },
    )
...
def poll_completion_with_task_guard(
    ...
    *,
    ...
    """Poll a turn, guarding the workspace around it.

    ``is_cancelled`` is threaded down to the poll so a stop lands while the
    turn is running rather than at the next phase boundary. The guard still
    runs on the way out either way -- a cancelled turn may well have left the
    checkout dirty, and that is exactly when a consumer wants to know.
    """
    targets = list(batch or [])
    # Only when there is a predicate. Passing `is_cancelled=None` is equivalent
    # for a client that supports it and a TypeError for one that does not --
    # and plenty do not: every existing test double and every client written
    # before this parameter existed.
    poll_kwargs = {}
    if is_cancelled is not None:
        parameters = inspect.signature(client.poll_completion).parameters.values()
        accepts_cancellation = any(
            parameter.name == "is_cancelled"
            or parameter.kind is inspect.Parameter.VAR_KEYWORD
            for parameter in parameters
        )
        if accepts_cancellation:
            poll_kwargs["is_cancelled"] = is_cancelled
    if not guard_state or not guard_before or not guard_after:
        return raise_for_provider_fallback_event(
            event=client.poll_completion(
                session_id, timeout=timeout_seconds, **poll_kwargs
            ),
            session_id=session_id,
            client=client,
            phase=phase,
    ...
    try:
        event = client.poll_completion(session_id, timeout=timeout_seconds, **poll_kwargs)
        return raise_for_provider_fallback_event(
            event=event,
            session_id=session_id,
            client=client,
