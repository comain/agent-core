...
def debug_log_dir() -> Path:
    """Where the consumer writes agent run logs, scanned for rate-limit evidence.

    Configurable because the writer is usually not this package. A consumer's
    CLI writes its own turn logs here and the harness reads them back looking
    for a provider's rate-limit response; if the two names diverge, nothing
    fails -- detection just silently loses a source and the run walks the whole
    provider chain instead of backing off.

    ``agent_debug_log_dir`` may be absolute, or a bare name resolved under the
    system temp directory.
    """
    configured = str(getattr(settings, "agent_debug_log_dir", "") or "").strip()
    if not configured:
        return Path(tempfile.gettempdir()) / "agent-run-logs"
    path = Path(configured).expanduser()
    return path if path.is_absolute() else Path(tempfile.gettempdir()) / path
...
def parse_rate_limit_payload(error_payload: Dict[str, Any]) -> Optional[Dict[str, Any]]:
    ...
    if not isinstance(data, dict):
        data = {}
    # OpenCode wraps provider failures as `{"name": "APIError", "data": {...}}`
    # with the message, status and headers directly on `data` -- not under
    # `data.error`, which is the only place this used to look. A token-pool
    # quota exhaustion therefore parsed as "not a rate limit", no model
    # fallback fired, and a twenty-minute agent turn ended as an unexplained
    # error instead of continuing on the next model in the chain.
    status_code = error_payload.get("statusCode")
    if status_code is None:
        status_code = data.get("statusCode")
    response_headers = (
        error_payload.get("responseHeaders") or data.get("responseHeaders") or {}
    )
    ...
    message = (
        ...
        or (data.get("message") if isinstance(data.get("message"), str) else "")
        or ""
    )
    ...
    is_rate_limited = (
        ...
        # Seen from the token-pool provider on 2026-08-26, as HTTP 400 with
        # `isRetryable: false`: "You have reached the limit." Nothing else in
        # this predicate matched it, so a genuine quota exhaustion was not
        # classified as one -- no model fallback fired, and a twenty-minute
        # agent turn ended as an unexplained error instead of continuing on
        # the next model in the chain.
        or "you have reached the limit" in message_lower
        or "reached the usage limit" in message_lower
...
def detect_rate_limit_in_logs(
    *,
    ...
    # Concurrent runs share these logs. Without a native conversation ID an
    # unrelated provider error is not evidence about this startup attempt.
    # Explicit errors on the process's own stream remain authoritative.
    if not session_id:
        return None
