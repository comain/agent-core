...
@respx.mock
def test_v2_create_session_uses_api_envelope(monkeypatch):
    monkeypatch.setattr("agent_core.config.settings.opencode_major", 2)
    client = OpenCodeClient(port=4096)
    respx.post("http://127.0.0.1:4096/api/session").mock(
        return_value=httpx.Response(200, json={"data": {"id": "ses_v2"}})
    )

    session_id = client.create_session(model_id="token-pool/gpt-5.5")
    assert session_id == "ses_v2"


@respx.mock
def test_v2_send_message_prompts_and_waits(monkeypatch):
    monkeypatch.setattr("agent_core.config.settings.opencode_major", 2)
    client = OpenCodeClient(port=4096)
    prompt = respx.post("http://127.0.0.1:4096/api/session/ses_v2/prompt").mock(
        return_value=httpx.Response(200, json={"data": {}})
    )
    wait = respx.post("http://127.0.0.1:4096/api/experimental/session/ses_v2/wait").mock(
        return_value=httpx.Response(204)
    )
    respx.get("http://127.0.0.1:4096/api/session/ses_v2/message").mock(
    ...
    client.send_message("ses_v2", "Hello")
    event = client.latest_completion("ses_v2")

    assert prompt.call_count == 1
    assert b'"text":"Hello"' in prompt.calls[0].request.content
