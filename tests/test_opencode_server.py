...
def test_server_strips_openai_api_key_for_openai_models(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr("agent_core.config.settings.opencode_provider", "openai")
...
def test_server_uses_custom_serve_cmd_when_configured(monkeypatch, tmp_path):
    monkeypatch.setattr(
        "agent_core.config.settings.opencode_serve_cmd",
...
def test_server_uses_ipv6_host_for_probe_and_cmd(monkeypatch, tmp_path):
    monkeypatch.setattr("agent_core.config.settings.opencode_host", "::1")
...
def test_server_v2_probes_api_info_with_password(monkeypatch, tmp_path):
    monkeypatch.setattr("agent_core.config.settings.opencode_major", 2)
    monkeypatch.setattr("agent_core.config.settings.opencode_server_password", "secret")
    observed = {}
    ...
    assert observed["url"] == "http://127.0.0.1:4096/api/info"
    assert observed["env"]["OPENCODE_SERVER_PASSWORD"] == "secret"
...
def test_server_keeps_openai_api_key_for_non_openai_models(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr("agent_core.config.settings.opencode_provider", "openrouter")
...
def test_server_persists_debug_log_when_print_logs_enabled(monkeypatch, tmp_path):
    monkeypatch.setattr("agent_core.config.settings.opencode_server_print_logs", True)
    monkeypatch.setattr("agent_core.config.settings.opencode_server_log_to_file", True)
...
def test_server_passes_ollama_host_when_configured(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr("agent_core.config.settings.opencode_provider", "ollama")
...
def test_server_passes_deepseek_credentials_when_configured(monkeypatch, tmp_path):
    monkeypatch.setattr("agent_core.config.settings.opencode_model", "deepseek/deepseek-v4-pro")
    monkeypatch.setattr("agent_core.config.settings.opencode_small_model", "deepseek/deepseek-v4-pro")
    monkeypatch.setattr("agent_core.config.settings.opencode_provider", "deepseek")
    monkeypatch.setattr("agent_core.config.settings.deepseek_api_key", "deepseek-secret")
