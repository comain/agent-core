...
def test_rate_limit_default_cooldown_is_sixty_seconds():
    tracker = ModelHealthTracker()
    assert tracker._cooldown_seconds("rate_limit", None) == 60


def test_timeout_cooldown_is_five_minutes():
    tracker = ModelHealthTracker()
    assert tracker._cooldown_seconds("timeout", None) == 5 * 60
...
def test_provider_auth_probe_quarantines_only_rejected_provider(monkeypatch):
    import httpx
    ...
    monkeypatch.setattr(
        "agent_core.harness.tiered_router.settings.opencode_provider_chain",
        "token-pool:gpt-5.6-sol;deepseek:deepseek/deepseek-v4-pro",
    )
    monkeypatch.setattr(
        "agent_core.harness.tiered_router.settings.opencode_provider_fallback_enabled",
        True,
    )
    monkeypatch.setattr(
    ...
    monkeypatch.setattr(
        ...
        "token-pool.token=valid;deepseek.token=invalid",
    )
    monkeypatch.setattr(
        "agent_core.harness.tiered_router.settings.opencode_model_api_cache_seconds",
        300,
    )
    ...
    assert [item.provider for item in candidates] == ["token-pool"]
    assert tiered_router._tracker.is_healthy("token-pool/gpt-5.6-sol") is True
    assert tiered_router._tracker.is_permanently_unhealthy(
