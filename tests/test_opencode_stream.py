...
def test_parser_tokens_accepts_flat_cache_aliases(parser):
    events = [
        _step_finish(
            reason="stop",
            tokens={
    ...
    assert result["cache"] == {"read": 80, "write": 5}
...
def test_parser_does_not_treat_javascript_line_429_as_rate_limit(parser):
    text = "at FileWatcher.state (/$bunfs/root/chunk.js:429:5418)"
    assert parser.detect_rate_limit_text(text) is False
...
def test_parser_requires_explicit_429_status_context(parser, text):
    assert parser.detect_rate_limit_text(text) is True
