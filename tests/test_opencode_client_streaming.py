...
def test_message_poll_requires_native_session_id():
    client = OpenCodeClient()
    process_id = client.create_session(model_id="token-pool/gpt-6-astra")
    with patch.object(client, "_http_request") as request:
        assert client.get_messages(process_id) == []
        assert client.get_messages(None) == []
...
def test_reusable_session_uses_private_model_config_when_repo_config_is_absent(tmp_path):
    config = HarnessConfig(
        opencode_provider_chain="token-pool:gpt-5.6-sol,kimi-k3",
        opencode_model="token-pool/gpt-5.6-sol",
        openai_base_url="https://token-pool.example/v1",
    )
    client = OpenCodeClient(repo_path=str(tmp_path), config=config)
    sid = client.create_session(model_id="token-pool/gpt-5.6-sol")
    client.send_message(sid, "Generate tests")
    captured = {}
    ...
    assert event["type"] == "completed"
    assert captured["config"]["provider"]["token-pool"]["models"] == {
        "gpt-5.6-sol": {
